...
wgpu::ShaderModule build_shader(const ShaderConfig& config) noexcept {
  ...
  if (config.exactScreenDepth) {
    ...
    vtxOutAttrs += fmt::format("\n    @location({}) screen_clip_position: vec2f,", vtxOutIdx++);
    vtxXfrAttrsPre += "\n    out.screen_clip_position = vec2f(dot(vec4f(mv_pos, 1.0), ubuf.screen_clip_x), dot(vec4f(mv_pos, 1.0), ubuf.screen_clip_y));";
  ...
  if (usesZTextureDepth || config.exactScreenDepth) {
    ...
    if (config.exactScreenDepth) {
      ...
      fragmentFn +=
          "\n    if (any(in.screen_clip_position < ubuf.screen_clip_rect.xy) || any(in.screen_clip_position >= ubuf.screen_clip_rect.zw)) { discard; }"
...
struct Uniform {{
    ...
    screen_clip_rect: vec4f,
    screen_clip_x: vec4f,
    screen_clip_y: vec4f,{0}
}};
@group(0) @binding(0)
