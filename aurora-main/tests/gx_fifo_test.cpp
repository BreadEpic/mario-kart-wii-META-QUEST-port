...
TEST_F(GXFifoTest, CameraOverlayRequiresBlendingAndNoDepthInteraction) {
  const auto info = aurora::gx::build_shader_info({});
  const auto classify = [&](bool perspective) {
    aurora::gfx::testing::reset_uniform_allocations();
    return aurora::gx::build_uniform(info, 0, aurora::gx::BindGroupRanges{},
        aurora::gx::FrameInterpolationDrawIdentity{}, perspective).replayLayout.cameraOverlay;
  };
  gxState().blendMode = GX_BM_BLEND;
  gxState().depthCompare = false;
  gxState().depthUpdate = false;
  EXPECT_TRUE(classify(true));
  EXPECT_FALSE(classify(false)); // Ordinary HUD still belongs on the hand.
  gxState().depthCompare = true;
  EXPECT_FALSE(classify(true)); // Transparent world geometry keeps stereo.
  gxState().depthCompare = false;
  gxState().depthUpdate = true;
  EXPECT_FALSE(classify(true));
  gxState().depthUpdate = false;
  gxState().blendMode = GX_BM_NONE;
  EXPECT_FALSE(classify(true)); // Opaque backgrounds are not screen overlays.
}
...
TEST_F(GXFifoTest, UniformRetainsViewportWindowForVrReplay) {
  ...
  gxState().renderViewport.left = 20.0f;
  gxState().renderViewport.top = 10.0f;
  gxState().renderViewport.width = 640.0f;
  gxState().renderViewport.height = 480.0f;
  gxState().renderScissor = {30, 40, 80, 90};
  ...
  EXPECT_EQ(ranges.replayLayout.projectionOffset, 128u);
  EXPECT_FLOAT_EQ(read_float(80), 30.0f);
  EXPECT_FLOAT_EQ(read_float(84), 40.0f);
  EXPECT_FLOAT_EQ(read_float(88), 110.0f);
  EXPECT_FLOAT_EQ(read_float(92), 130.0f);
  EXPECT_FLOAT_EQ(read_float(96), 320.0f);
  EXPECT_FLOAT_EQ(read_float(108), 340.0f);
  EXPECT_FLOAT_EQ(read_float(116), -240.0f);
  EXPECT_FLOAT_EQ(read_float(124), 250.0f);
}
