...
constexpr size_t kStagedUniformBytes = 144 + sizeof(Mat4x4<float>) + sizeof(Mat3x4<float>) * (MaxPostexMtx + MaxPnMtx);
...
UniformRanges build_uniform(const ShaderInfo& info, u32 vtxStart, const BindGroupRanges& ranges,
  ...
  // Original EFB pixel coordinates remain available after HUD reprojection.
  // Fragment clipping reproduces GX scissoring on a tilted hand-held panel.
  const auto& scissor = g_gxState.renderScissor;
  stage_f32(static_cast<float>(scissor.x));
  stage_f32(static_cast<float>(scissor.y));
  stage_f32(static_cast<float>(scissor.x + scissor.width));
  stage_f32(static_cast<float>(scissor.y + scissor.height));
  for (size_t i = 0; i < 4; ++i)
    stage_f32(effectiveProj.m0[i] * viewport.width * 0.5f + effectiveProj.m3[i] * (viewport.left + viewport.width * 0.5f));
  for (size_t i = 0; i < 4; ++i)
    stage_f32(-effectiveProj.m1[i] * viewport.height * 0.5f + effectiveProj.m3[i] * (viewport.top + viewport.height * 0.5f));
  if (info.lineMode != 0) {
  ...
  const UniformReplayLayout replayLayout{
      ...
      .cameraOverlay = perspective && blends && !g_gxState.depthCompare && !g_gxState.depthUpdate,
  };
